 ...
 *
 * Uses `SessionManager.forkFrom(sourcePath, cwd)` (session-manager.ts in the
 * pinned OMP version): copies the source session's entries and artifacts into
 * a brand new session file with `parentSession` set in the header, without
 * ever making the source live. The source file is only READ, so forking a
 * session that is currently open in a worker is safe — the fork is a snapshot
 * at fork time.
 *
